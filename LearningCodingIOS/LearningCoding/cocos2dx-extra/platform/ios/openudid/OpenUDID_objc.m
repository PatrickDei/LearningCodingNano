...
@interface OpenUDID_objc (Private)
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                ...
                result[12], result[13], result[14], result[15],
                     (u_int32_t)(arc4random() % UINT32_MAX)];
    }
...
//
+ (NSString*) value {
    return [OpenUDID_objc valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [OpenUDID_objc _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [OpenUDID_objc _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [OpenUDID_objc _setDict:localDict forPasteboard:slotPB];
    }
