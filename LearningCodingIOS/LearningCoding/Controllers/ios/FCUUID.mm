...
static NSString *_uuidForDevice;
...
- (NSString *)uuidForDevice
{
  ...
  return [self uuidForDeviceUsingValue:nil forceOverwrite:NO];
}

- (NSString *)uuidForDeviceUsingValue:(NSString *)uuidValue forceOverwrite:(BOOL)forceOverwrite
{
  // also known as udid/uniqueDeviceIdentifier but this doesn't persists to system reset

  NSString *uuidForDeviceInMemory = _uuidForDevice;

  if ([self uuidValueIsValid:uuidValue] && forceOverwrite) {
  ...
  if ([self uuidValueIsValid:uuidValue] && ![_uuidForDevice isEqualToString:uuidValue] && !forceOverwrite) {
    [NSException raise:@"Cannot overwrite uuidForDevice"
                format:@"uuidForDevice has already been created and cannot be overwritten."];
  }
...
{
  return [self uuidForDeviceMigratingValueForKey:key
                                         service:nil
                                     accessGroup:nil
...
- (NSString *)uuidForDeviceMigratingValueForKey:(NSString *)key
                                ...
                                commitMigration:(BOOL)commitMigration
                                 forceMigration:(BOOL)forceMigration
{
  ...
  if (uuidToMigrate == nil) {
    ...
    if (uuidToMigrate == nil) {
      NSDictionary *dict = [[NSUserDefaults standardUserDefaults] valueForKey:key];
      if (dict != nil) {
        uuidToMigrate = [dict valueForKey:key];
      }
    }
  }
  ...
  if (commitMigration) {
    if ([self uuidValueIsValid:uuidToMigrate]) {
      return [self uuidForDeviceUsingValue:uuidToMigrate forceOverwrite:forceMigration];
    } else {
...
- (void)uuidsOfUserDevices_iCloudInit
{
  if (NSClassFromString (@"NSUbiquitousKeyValueStore")) {
    ...
    if (iCloud) {
      ...
      // if keychain contains more device identifiers than icloud, maybe that icloud has been empty,
      // so re-write these identifiers to iCloud
  ...
  @synchronized (self)
  {
    ...
    if ([uuidsSet count] != uuidsCount) {
      ...
      NSDictionary *userInfo =
          [NSDictionary dictionaryWithObject:[self uuidsOfUserDevices] forKey:@"uuidsOfUserDevices"];
      [[NSNotificationCenter defaultCenter] postNotificationName:FCUUIDsOfUserDevicesDidChangeNotification
                                                          object:self
                                                        userInfo:userInfo];
    }
  }
...
- (BOOL)uuidValueIsValid:(NSString *)uuidValue
{
  ...
  return (uuidValue != nil && (uuidValue.length == 32 || uuidValue.length == 36 || uuidValue.length == 40));
}

- (void)clearUdid { _uuidForDevice = nil; }

+ (NSString *)uuid { return [[self sharedInstance] uuid]; }

+ (NSString *)uuidForKey:(id<NSCopying>)key { return [[self sharedInstance] uuidForKey:key]; }

+ (NSString *)uuidForSession { return [[self sharedInstance] uuidForSession]; }

+ (NSString *)uuidForInstallation { return [[self sharedInstance] uuidForInstallation]; }

+ (NSString *)uuidForVendor { return [[self sharedInstance] uuidForVendor]; }

+ (NSString *)uuidForDevice { return [[self sharedInstance] uuidForDevice]; }

+ (NSString *)uuidForDeviceMigratingValueForKey:(NSString *)key commitMigration:(BOOL)commitMigration
{
  return [[self sharedInstance] uuidForDeviceMigratingValueForKey:key
                                                          service:nil
                                                      accessGroup:nil
                                                  commitMigration:commitMigration
                                                   forceMigration:NO];
...
+ (NSArray *)uuidsOfUserDevices { return [[self sharedInstance] uuidsOfUserDevices]; }

+ (BOOL)uuidValueIsValid:(NSString *)uuidValue { return [[self sharedInstance] uuidValueIsValid:uuidValue]; }

+ (BOOL)checkIfUdidExists
{
  NSString *udid = [UICKeyChainStore stringForKey:_uuidForDeviceKey];
  if (udid == nil) {
    udid = [[NSUserDefaults standardUserDefaults] stringForKey:_uuidForDeviceKey];
  }
  return udid != nil && udid.length > 0;
}

+ (void)clearUuidForDevice
{
  [[NSUserDefaults standardUserDefaults] removeObjectForKey:_uuidForDeviceKey];
  [[NSUserDefaults standardUserDefaults] synchronize];
  [UICKeyChainStore removeItemForKey:_uuidForDeviceKey];
  _uuidForDevice = nil;
}
