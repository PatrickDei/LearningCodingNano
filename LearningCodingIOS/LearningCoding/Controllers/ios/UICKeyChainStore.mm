...
{
  if (!_defaultService) {
    _defaultService = @"com.nanobitsoftware.fcuuid";
  }

  return _defaultService;
}

+ (void)setDefaultService:(NSString *)defaultService { _defaultService = defaultService; }

#pragma mark -

+ (UICKeyChainStore *)keyChainStore { return [[self alloc] initWithService:nil accessGroup:nil]; }
...
+ (UICKeyChainStore *)keyChainStoreWithServer:(NSURL *)server protocolType:(UICKeyChainStoreProtocolType)protocolType
{
  return [[self alloc] initWithServer:server
                   ...
                   authenticationType:UICKeyChainStoreAuthenticationTypeDefault];
}

+ (UICKeyChainStore *)keyChainStoreWithServer:(NSURL *)server
                                 protocolType:(UICKeyChainStoreProtocolType)protocolType
                           authenticationType:(UICKeyChainStoreAuthenticationType)authenticationType
{
...
- (instancetype)init { return [self initWithService:[self.class defaultService] accessGroup:nil]; }

- (instancetype)initWithService:(NSString *)service { return [self initWithService:service accessGroup:nil]; }
...
- (void)commonInit { _accessibility = UICKeyChainStoreAccessibilityAfterFirstUnlock; }

#pragma mark -

+ (NSString *)stringForKey:(NSString *)key { return [self stringForKey:key service:nil accessGroup:nil error:nil]; }
...
{
  return
      [self setString:value forKey:key service:service accessGroup:nil genericAttribute:genericAttribute error:error];
}
...
+ (NSData *)dataForKey:(NSString *)key { return [self dataForKey:key service:nil accessGroup:nil error:nil]; }
...
- (NSString *)stringForKey:(id)key { return [self stringForKey:key error:nil]; }
...
- (NSData *)dataForKey:(NSString *)key { return [self dataForKey:key error:nil]; }
...
+ (BOOL)removeItemForKey:(NSString *)key { return [self removeItemForKey:key service:nil accessGroup:nil error:nil]; }
...
+ (BOOL)removeAllItems { return [self removeAllItemsForService:nil accessGroup:nil error:nil]; }
...
- (BOOL)removeItemForKey:(NSString *)key { return [self removeItemForKey:key error:nil]; }
...
- (BOOL)removeAllItems { return [self removeAllItemsWithError:nil]; }
...
- (NSString *)objectForKeyedSubscript:(NSString<NSCopying> *)key { return [self stringForKey:key]; }
...
- (NSArray *)allItems { return [self.class prettify:[self itemClassObject] items:[self items]]; }
...
- (void)sharedPasswordWithCompletion:(void (^)(NSString *account, NSString *password, NSError *error))completion
{
  ...
  if (domain.length > 0) {
    ...
    NSError *error = [self.class argumentError:NSLocalizedString (@"the server property must not to be nil, should use "
...
- (void)setSharedPassword:(NSString *)password
               forAccount:(NSString *)account
               completion:(void (^)(NSError *error))completion
{
  NSString *domain = self.server.host;
  if (domain.length > 0) {
    SecAddSharedWebCredential ((__bridge CFStringRef)domain, (__bridge CFStringRef)account,
                               (__bridge CFStringRef)password, ^(CFErrorRef error) {
                                 if (completion) {
...
+ (void)requestSharedWebCredentialForDomain:(NSString *)domain
                                 ...
                                 completion:(void (^)(NSArray *credentials, NSError *error))completion
{
  SecRequestSharedWebCredential ((__bridge CFStringRef)domain, (__bridge CFStringRef)account,
...
+ (NSString *)generatePassword { return CFBridgingRelease (SecCreateSharedWebCredentialPassword ()); }
...
- (NSString *)debugDescription { return [NSString stringWithFormat:@"%@", [self items]]; }
  ...
  if (_authenticationPolicy && accessibilityObject) {
    if (floor (NSFoundationVersionNumber) > floor (iOS_7_1_or_10_9_2)) { // iOS 8+ or OS X 10.10+
      ...
      SecAccessControlRef accessControl = SecAccessControlCreateWithFlags (
          kCFAllocatorDefault, accessibilityObject, (SecAccessControlCreateFlags)_authenticationPolicy, &securityError);
      if (securityError) {
...
#if TARGET_OS_IPHONE
      NSLog (@"%@", @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS "
                    @"versions prior to 8.0.");
#else
      NSLog (@"%@", @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X "
                    @"versions prior to 10.10.");
#endif
    } else {
...
+ (NSError *)argumentError:(NSString *)message
{
  NSError *error = [NSError errorWithDomain:UICKeyChainStoreErrorDomain
                                       code:UICKeyChainStoreErrorInvalidArguments
...
{
  NSError *error =
      [NSError errorWithDomain:UICKeyChainStoreErrorDomain code:-67594
                      userInfo:@{NSLocalizedDescriptionKey : message}];
  NSLog (@"error: [%@] %@", @(error.code), error.localizedDescription);
  return error;
}
...
#endif
  NSError *error =
      [NSError errorWithDomain:UICKeyChainStoreErrorDomain code:status
                      userInfo:@{NSLocalizedDescriptionKey : message}];
...
{
  NSError *error =
      [NSError errorWithDomain:UICKeyChainStoreErrorDomain code:-99999
                      userInfo:@{NSLocalizedDescriptionKey : message}];
  NSLog (@"error: [%@] %@", @(error.code), error.localizedDescription);
  return error;
}
