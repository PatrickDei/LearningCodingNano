...
+ (NSMutableDictionary *)_getDictFromPasteboard:(id)pboard
{
  ...
  return [NSMutableDictionary
      dictionaryWithDictionary:(item == nil || [item isKindOfClass:[NSDictionary class]]) ? item : nil];
}
...
// After that, the caching/pasteboard/redundancy mechanism inside [OpenUDID value] returns a persistent and cross
// application OpenUDID
//
+ (NSString *)_generateFreshOpenUDID
{
...
//    }
  ...
  /*
      ...
      with the word "Support", and then assigning _openUDID with the only identifier method called on the sharedManager
of that new class... don't forget to add
      the framework to your project!

#if TARGET_OS_IPHONE
  if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"6.0")) {
      _openUDID = [[[UIDevice currentDevice] identifierForA_______] UUIDString];
# error                                                         ^ read comments above, fix accordingly, and remove this
#error line
  }
#endif
  ...
  if (_openUDID == nil) {
    ...
    _openUDID =
      [NSString stringWithFormat:@"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%08lx", result[0],
       result[1], result[2], result[3], result[4], result[5], result[6], result[7],
       result[8], result[9], result[10], result[11], result[12], result[13], result[14],
       result[15], (unsigned long)(arc4random () % NSUIntegerMax)];
  }
...
// It will return the null udid (forty zeros) if the user has somehow opted this app out (this is subject to 3rd party
// implementation)
// Otherwise, it will register the current app and return the OpenUDID
//
+ (NSString *)value { return [OpenUDID valueWithError:nil]; }
...
+ (NSString *)valueWithError:(NSError **)error
{
  if (kOpenUDIDSessionCache != nil) {
    if (error != nil)
      *error = [NSError errorWithDomain:kOpenUDIDDomain
                               ...
                               userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",
                                                                                   @"description", nil]];
  ...
  // This local copy contains a copy of the openUDID, myRedundancySlotPBid (and unused in this block, the local
  // bundleid, and the timestamp)
  //
  id localDict = [defaults objectForKey:kOpenUDIDKey];
  if ([localDict isKindOfClass:[NSDictionary class]]) {
    localDict =
        [NSMutableDictionary dictionaryWithDictionary:localDict]; // we might need to set/overwrite the redundancy slot
  ...
  // The idea behind this is to both multiple and redundant representations of OpenUDIDs, as well as serve as
  // placeholder for potential opt-out
  //
  ...
  // if openUDID was not retrieved from the local preferences, then let's try to get it from the frequency dictionary
  // above
  //
  if (openUDID == nil) {
  ...
  if (availableSlotPBid != nil &&
      (myRedundancySlotPBid == nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
  ...
  // If the UIPasteboard external representation marks this app as opted-out, then to respect privacy, we return the
  // ZERO OpenUDID, a sequence of 40 zeros...
  // This is a *new* case that developers have to deal with. Unlikely, statistically low, but still.
  // To circumvent this and maintain good tracking (conversion ratios, etc.), developers are invited to calculate how
  // many of their users have opted-out from the full set of users.
  // This ratio will let them extrapolate convertion ratios more accurately.
  //
  if (optedOut) {
    if (error != nil)
      *error =
          [NSError errorWithDomain:kOpenUDIDDomain
                              code:kOpenUDIDErrorOptedOut
                          userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                                     [NSString stringWithFormat:@"Application with unique id %@ is "
                                                                                @"opted-out from OpenUDID as of %@",
                                                                                appUID, optedOutDate],
                                                     @"description", nil]];
  ...
  if (error != nil) {
    if (isCompromised)
      *error = [NSError
          errorWithDomain:kOpenUDIDDomain
                     code:kOpenUDIDErrorCompromised
                 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Found a discrepancy between stored OpenUDID "
                                                                     @"(reliable) and redundant copies; one of the "
                                                                     @"apps on the device is most likely corrupting "
                                                                     @"the OpenUDID protocol",
                                                                     @"description", nil]];
    else
      *error = [NSError errorWithDomain:kOpenUDIDDomain
                                   code:kOpenUDIDErrorNone
                               userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID succesfully retrieved",
                                                                                   @"description", nil]];
  }
...
+ (NSString *)getOpenUdidFromUserDefaults
{
  // Do we have a local copy of the OpenUDID dictionary?
  // This local copy contains a copy of the openUDID, myRedundancySlotPBid (and unused in this block, the local
  // bundleid, and the timestamp)
...
+ (BOOL)checkIfUdidExists
{
  ...
  if (appUid == nil) {
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
      ...
      appUid = [localDict valueForKey:kOpenUDIDAppUIDKey];
      [defaults setValue:appUid forKey:kOpenUDIDAppUIDKey];
    }
  }
  if (appUid != nil) {
    // Try to get local open udid from user defaults
    NSString *openUdid = [OpenUDID getOpenUdidFromUserDefaults];

    NSArray *sortedUdidArray = [OpenUDID getSavedUdidsArray];

    // if openUDID was not retrieved from the local preferences, then let's try to get it from the frequency dictionary
    // above
    NSString *mostReliableOpenUDID =
        (sortedUdidArray != nil && [sortedUdidArray count] > 0) ? [sortedUdidArray lastObject] : nil;
    OpenUDIDLog (@"Freq Dict = %@\nMost reliable %@", frequencyDict, mostReliableOpenUDID);

    if (openUdid != nil || mostReliableOpenUDID != nil) {
      udidExists = YES;
    }
  }
...
+ (NSString *)findAvailablePasteboardSlot
{
  // Find available pasteboard slot
  NSString *availableSlotPBid = nil;
  for (int n = 0; n < kOpenUDIDRedundancySlots; n++) {
    NSString *slotPBid = [NSString stringWithFormat:@"%@%d", kOpenUDIDSlotPBPrefix, n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
  ...
  return availableSlotPBid;
}

+ (void)saveUdid:(NSString *)udid
{
  NSString *availableSlotPBid = [self findAvailablePasteboardSlot];
  if (availableSlotPBid != nil) {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
...
+ (void)clearUdid
{
  ...
  for (int n = 0; n < kOpenUDIDRedundancySlots; n++) {
    ...
    if (slotPB != nil) {
      ...
      OpenUDIDLog (@"SlotPB dict = %@", dict);
      if (oudid != nil) {
        [OpenUDID _setDict:nil forPasteboard:slotPB];
      }
    }
  }

  NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
  [defaults removeObjectForKey:kOpenUDIDKey];
  [defaults removeObjectForKey:kOpenUDIDAppUIDKey];
  kOpenUDIDSessionCache = nil;
}
