...
bool CCJSONConverter::sortKeys = false;
...
char * CCJSONConverter::strFrom(CCDictionary *dictionary, bool sortKeys)
{
    CCAssert(dictionary, "CCJSONConverter:can not convert a null pointer");
    CCJSONConverter::sortKeys = sortKeys;
...
bool CCJSONConverter::isStrJSON(const char * str)
{
...
CCArray* CCJSONConverter::arrayFrom(const char *str)
{
    cJSON * json = cJSON_Parse(str);
    //    CCAssert(json && json->type==cJSON_Array, "CCJSONConverter:wrong json format");
...
{
    if (CCJSONConverter::sortKeys)
    {
        auto keysSet = dictionary->allKeysSet();
        for (auto &key : keysSet)
        {
            CCObject * obj = dictionary->objectForKey(key);
            cJSON * jsonItem = getObjJson(obj);
            cJSON_AddItemToObject(json, key.c_str(), jsonItem);
        }
    }
    else
    {
